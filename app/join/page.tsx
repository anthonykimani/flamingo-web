...
const JoinGame = () => {
  ...
  // Pre-fill nickname from World App username
  useEffect(() => {
    if (isWorldApp && username && !nickname) {
      setNickname(username)
    }
  }, [isWorldApp, username, nickname])
    ...
    switch (stepper) {
      case JoinGameStep.ENTERGAMEPIN:
        ...
        try {
          ...
          // Skip all blockchain tx — Hangouts mode is free
          // Join game via WebSocket directly
              ...
              <div className="h-full flex flex-col justify-center md:items-center p-1 sm:p-3">
                ...
                <div className='flex flex-col justify-end mt-4 gap-2'>
                  ...
                  <Input
                    name='nickname'
                    variant="default"
                    placeholder={isWorldApp ? 'Nickname (from World ID)' : 'Choose Nickname'}
                    value={nickname}
                    onChange={(e) => setNickname(e.target.value)}
                    maxLength={20}
                  />
                  {isWorldApp && username && (
                    <p className='text-white text-center text-sm bg-black/40 p-1 rounded'>
                      Using World ID profile: {username}
                    </p>
                  )}
                  <Button
                    variant="active"
