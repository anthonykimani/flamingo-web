...
export async function addQuiz(gameData: IQuiz): Promise<IResponse> {
  const response = await Http.post(
    `${apiOptions.endpoints.gameService}/quizzes/createQuiz`,
    gameData
  )
...
export async function addAgentQuiz(prompt: string): Promise<IResponse> {
  const response = await Http.post(`${apiOptions.endpoints.gameService}/quizzes/createAgentQuiz`, {
    prompt: prompt,
  })
...
export async function createGameSession(config: string | IGameConfig): Promise<IResponse> {
  ...
  const requestData = typeof config === 'string' ? { quizId: config } : config
...
export async function joinGame(gamePin: string): Promise<IResponse> {
  const response = await Http.post(`${apiOptions.endpoints.gameService}/games/join`, { gamePin })
